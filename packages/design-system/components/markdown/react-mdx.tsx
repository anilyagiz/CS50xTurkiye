...
export const reactMdxComponents: Options["components"] = {
  ...
  p: memo(
    ({ ...props }) => <Paragraph data-CS50xTürkiye="paragraph" {...props} />,
    (p, n) => sameClassAndNode(p, n)
  ),
  ol: memo(
    ({ ...props }) => (
  ...
  em: memo(
    ({ ...props }) => (
      <em className="font-medium" data-CS50xTürkiye="italic" {...props} />
    ),
    (p, n) => sameClassAndNode(p, n)
  ),
  strong: memo(
    ({ ...props }) => (
      <strong className="font-medium" data-CS50xTürkiye="bold" {...props} />
    ),
    (p, n) => sameClassAndNode(p, n)
  ),
  blockquote: memo(
  ...
  a: memo(
    ({ ...props }) => <Anchor data-CS50xTürkiye="anchor" {...props} />,
    (p, n) => sameClassAndNode(p, n)
  ),
  table: memo(
    ({ children, ...props }) => (
  ...
  hr: memo(
    ({ ...props }) => (
      <hr className="my-4 border-border" data-CS50xTürkiye="hr" {...props} />
    ),
    (p, n) => sameClassAndNode(p, n)
  ),
  tbody: memo(
    ({ children, ...props }) => (
      <TableBody className="bg-muted/40" data-CS50xTürkiye="table-body" {...props}>
        {filterWhitespaceNodes(children)}
      </TableBody>
    ),
    (p, n) => sameClassAndNode(p, n)
  ...
  code: memo(
    ({ node, children, className, ...props }) => {
      ...
      if (isInlineMath) {
        return (
          <InlineMath data-CS50xTürkiye="math-inline">{String(children)}</InlineMath>
        );
      }
